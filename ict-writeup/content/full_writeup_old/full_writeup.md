...
![][Caught_between_a_rock_and_a_hard_place]
...
Navigating an "Idea Maze"[^fn7] is a powerful analogy for a process that involves a series of hard-to-reverse decisions under uncertainty. Idea Mazes usually show up in the context of startups and the decisions an entrepreneur navigates to create a company --- "should we sell to businesses or consumers? Build a web application or iPhone App first? etc." Despite being associated with startups, there is nothing about the analogy of an idea maze that restricts it to a particular domain; it can also be used in engineering, research, or in this case, building new institutional structures.![][PastedGraphic-1]

Image Credit: [Balaji S. Srinivasan](https://spark-public.s3.amazonaws.com/startup/lecture_slides/lecture5-market-wireframing-design.pdf)
...
# Things that are too engineering-heavy for academia and too researchy for startups #
...
![][ScientificInquiryvengineeringdesign]Schematic of scientific inquiry vs engineering design
...
Autocatalytic reactions are (chemical) reactions that produce at least one of their own reactants. ![][1920px-Formose]Autocatalytic reactions are complicated!
...
![][4767B576-22D0-4824-BF0A-31264C2F2EC7]
...
![][image1-1]The structure of a typical VC firm[^fn122]
...
![iu.png](iu.png)This person looks like they know what they're doing.
...
1. [Industrial labs enabled targeted piddling around][]. 
...
1. Industrial labs enabled high-collaboration research work among larger and more diverse groups of people than in academia or startups.
...
1. Industrial Labs enabled smooth transitions of technologies between different readiness levels --- they cared about both about novelty *and* scale. 
...
1. Industrial Labs provided a default customer for process improvements and default scale for products.
...
1. Industrial Labs often provided a precise set of problems and feedback loops about whether solutions actually solved those problems.
...
1. Industrial Labs provided a first-class alternative to academia where people could still participate in the scientific enterprise. 
...
1. Corporate labs enabled continuous work on projects over 6+ year timescales.
...
![][PastedGraphic6]You don't often see this kind of program design any more. Was it never useful, now outdated, or a lost art?
...
![][PastedGraphic8]What might a Wardley-inspired roadmap for describing the complete structural connectivity of the human brain look like?
...
## Transition from Maze Interior to Plan ##
...
Work might not be able to get through a grant committee for several reasons: it doesn't fit into a neat bucket; it isn't directly related to research that the performer has done before[^fn253]; or it just doesn't have strong evidence that it will succeed. 
...
Some tactics that we can test to 'onboard' collaborators include: small workshops of high-potential possible performers; spending a lot of work building trust with individual researchers; [^fn254]working to build a long-term community that will continue to be valuable to researchers after any specific project. 
...
DARPA PMs use seedling projects to 'acid test' the riskiest pieces of a program idea.[^fn255] I'm confident that PARPA will be able to do something similar for our programs but it's still a hypothesis we need to explicitly test. It is extremely hard to call out in advance which pieces of evidence would compellingly suggest that a crazy idea is at least possible. 
...
Below are some top-of-mind examples[^fn256] of speculative seedling projects to prime your intuition pump. Binding constraints They might not be feasible for many reasons!
...
One sub-hypothesis of 'graduating programs' is that there is a lot of 'pre-work' that can be done during the course of a program to maximize its impact after it concludes. It would be easy to focus monomaniacally on getting to some tremendous milestone, reach it, and then look around and ask "ok, what do we do with this now?" Or worse, assert "well, our job here is done, it's up to others to carry the torch forward!" Instead, PMs could act a bit like curling sweepers --- altering the friction in front of the program ever so slightly so it ends up in the best place possible. ![YOG-2016-UAlberta-Middleton-Burgess-sweeping-FI.png](YOG-2016-UAlberta-Middleton-Burgess-sweeping-FI.png)
...
Spinning out[^fn260] as a company is only one of several ways that programs can 'graduate.' However it's worth talking about them directly because startups dominate so much of the cultural zeitgeist and potentially play a unique role in PARPA's business plans.  
...
While PARPA will hopefully be able to accomplish many important things, it will be structurally unable to do *all* the important things. Companies can conceivably[^fn262] monopolize an entire market and provide all the search, steel, cat videos, or diapers in the world. By contrast, DARPA's tiny and flat structure likely plays a key role in their success[^fn263] and any organization riffing on its model will see diminishing returns beyond its size.[^fn264] This may be similar to what you see in venture capital firms and many private equity firms --- small firms are able to get crazy returns on outlier results while large ones revert to the mean.
...
Contingent factors can always kill a given organization even if it gets the model correct. Or perhaps like ARDC[^fn265], arguably the first VC firm, you get the idea and some pieces correct but screw up others (like legal structures) in an ultimately fatal way.
...
However, while an aggregate of all of PARPA's activities will not be profitable, it's a reasonable (but still risky) hypothesis that a portfolio of companies spun out of PARPA could end up doing well. Regardless of profit, [PARPA will eventually spin out companies][] because companies can be an effective mechanism for technology diffusion. It's an even safer bet that if PARPA is doing something useful, there will end up being many profit-generating organizations that can trace their DNA back to PARPA. But let's ignore the latter for now.[^fn266]
...
![][Pokemon-Charmander-PNG-Transparent-Image]
...
* A project to develop precise actuators for a DNA-based printer. Perhaps an expansion on William Shih's work. [^fn282]
* A project to develop protein building blocks (probably based on coiled-coil proteins[^fn283]1. that could be used in a DNA-based printer.
...
* A project to poke at how Metal Oxide Frameworks [^fn284]might play with the other projects.
* A project to look at how DNA/protein metal 'casting' [^fn285]might play with the other projects.
...
* *Serve as a jumping-off point for eventually extremely impactful people.* History is full of largely-forgotten 'sceniuses'[^fn289] that quietly lie in the past of an uncanny number of impactful individuals. Creating a even a short-lived scenius, like the Rad Lab or Willow Garage would be a worthy result.
...
**Experimentally verify Clark's First Law**[^fn294] is neat because it implicitly lays out an organizational plan of attack (find out what elder preeminent scientists say is possible and impossible!) But it's super obscure and not a great rallying cry.
...
Finally, I want to propose a specific path through the maze --- PARPA. We can look at PARPA[^fn295]'s potential path through the idea maze as a sequence of hypotheses --- junctures that decrease your risk of failing for each one you successfully pass. Bundled together, PARPA's path through the maze looks like:
...
[PastedGraphic2]: PastedGraphic2.png width=284px height=284px

[PastedGraphic1]: PastedGraphic1.png width=222px height=222px

[PastedGraphic]: PastedGraphic.png width=270px height=270px

[Caught_between_a_rock_and_a_hard_place]: Caught_between_a_rock_and_a_hard_place.jpg width=239px height=270px

[PastedGraphic-1]: PastedGraphic.png width=270px height=178px

[iu]: iu.jpg width=196px height=270px

[PastedGraphic1-1]: PastedGraphic1.png width=270px height=263px

[PastedGraphic2-1]: PastedGraphic2.png width=270px height=263px

[PastedGraphic3]: PastedGraphic3.png width=270px height=262px

[PastedGraphic14]: PastedGraphic14.tiff

[PastedGraphic13]: PastedGraphic13.png width=725px height=452px

[PastedGraphic-2]: PastedGraphic.png width=239px height=165px

[ScientificInquiryvengineeringdesign]: ScientificInquiryvengineeringdesign.png width=1920px height=1080px

[image1]: image1.png width=468px height=298px

[fusion_never]: fusion_never.png width=268px height=201px

[1920px-Formose]: 1920px-Formose.png width=1440px height=1220px

[4767B576-22D0-4824-BF0A-31264C2F2EC7]: 4767B576-22D0-4824-BF0A-31264C2F2EC7.png

[iu-1]: iu.jpg width=271px height=181px

[PastedGraphic11]: PastedGraphic11.png width=699px height=568px

[image1-1]: image1.png width=468px height=291px

[PastedGraphic6]: PastedGraphic6.png width=223px height=270px

[PastedGraphic8]: PastedGraphic8.png width=270px height=243px

[PastedGraphic5]: PastedGraphic5.png width=270px height=175px

[PastedGraphic-3]: PastedGraphic.png width=149px height=269px

[PastedGraphic9]: PastedGraphic9.png width=929px height=533px

[PastedGraphic10]: PastedGraphic10.png width=813px height=566px

[iu-2]: iu.jpg width=317px height=235px

[PastedGraphic-4]: PastedGraphic.png width=873px height=436px

[Pokemon-Charmander-PNG-Transparent-Image]: Pokemon-Charmander-PNG-Transparent-Image.png width=243px height=270px

[iu-3]: iu.png width=270px height=270px

[iu-4]: iu.png width=270px height=214px

[PastedGraphic12]: PastedGraphic12.png width=492px height=583px
...
[^fn122]: From [VC Funds 101: Understanding Venture Fund Structures, Team Compensation, Fund Metrics and Reporting \| by Ahmad Takatkah \| VCpreneur](https://vcpreneur.com/vc-funds-101-understanding-venture-fund-structures-team-compensation-fund-metrics-and-reporting-152b02e8504a)
...
[^fn250]: There are plenty of people who have no idea what they're talking with great ideas that would change the world if only they could find someone to implement them.

[^fn251]: The counterexamples that immediately popped into your head are the result of saliency bias. It's easy to recall people who openly want to work on things beyond their reach but obviously hard to recall the people who never talk about.

[^fn252]: I think credentialism is bad and that most people can retrain to do most things eventually if they try hard. However, success on a technically-hard non-software research project starting from scratch without guidance seems ... unlikely.

[^fn253]: The tension between expertise and ambition strikes again!

[^fn254]: \<sidenote\> Warren Weaver, the Rockefeller Foundation program officer who funded early genetics work expands on this point in his [notes for other program officers](https://rockfound.rockarch.org/digital-library-listing/-/asset_publisher/yYxpQfeI4W8N/content/n-s-notes-on-officers-techniques).

[^fn255]: \<sidenote\> See [DARPA PMs use seedling projects to 'acid test' the riskiest pieces of a program idea](https://benjaminreinhardt.com/wddw#darpa_pms_use_seedling_projects_to_%E2%80%98acid_test%E2%80%99_the_riskiest_pieces_of_a_program_idea)

[^fn256]: \<sidenote\> The last four are directly from Adam Marblestone.

[^fn257]: \<sidenote\> See [A large part of a DARPA program manager's job is focused network building](https://benjaminreinhardt.com/wddw#a_large_part_of_a_darpa_program_manager%E2%80%99s_job_is_focused_network_building)

[^fn258]: See question \#4 of The Heilmeier Catechism: " Who cares? If you are successful, what difference will it make?"

[^fn259]: \<sidenote\> In [Reinventing Discovery](https://www.goodreads.com/book/show/11667578-reinventing-discovery), Michael Nielsen elegantly describes this process as 'designed serendipity'

[^fn260]: I prefer the imagery invoked by 'budding off' companies instead of spinning out, but the convention is strong with this one.

[^fn261]: \<sidenote\> See [Default Alive or Default Dead](http://paulgraham.com/aord.html)

[^fn262]: Though they rarely do in practice

[^fn263]: \<sidenote\> See [DARPA is relatively tiny and flat](https://benjaminreinhardt.com/wddw#darpa_is_relatively_tiny_and_flat)

[^fn264]: 1974 Bell Labs and 2020 DARPA have uncannily similar budgets ($3.6B and $2.8B in 2020 dollars) - there is not clear causal linkage but it is suggestive.

[^fn265]: \<sidenote\> For much more about ARDC see [Creative Capital: Georges Doriot and the Birth of Venture Capital](https://www.goodreads.com/book/show/3176553-creative-capital)

[^fn266]: If every company whose core business depends on technology that can be traced back to DARPA gave some tiny percent of their revenue to DARPA, it would fund DARPA many times over. (And in a way they do, via taxes in the US)

[^fn267]: \<sidenote\> See [Productive Uncertainty](http://reactionwheel.net/2020/11/productive-uncertainty.html)

[^fn268]: As with any metric, it will be very imperfect. It's not actually possible to convert between four years of service and $200k. Both are essential and uninterchangable.

[^fn269]: \<sidenote\> Evolutionary systems (may) go through long periods of slow change punctuated by periods of rapid change. See [Punctuated equilibria: an alternative to phyletic gradualism](https://www.blackwellpublishing.com/ridley/classictexts/eldredge.pdf)

[^fn270]: \<sidenote\> For some more complex and well-thought-out tranched funding schemes, see [Funding Long Shots](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3058472).

[^fn271]: **While a DARPA-riff will need at least seven programs to demonstrate viability, we could design the remaining programs once we've moved to Form 2.**

[^fn272]: I've gotten different response rates sending emails with the slightly different wording "potential privately funded research program ..." and just "potential research program ..."

[^fn273]: \<sidenote\> See[ Focused Research Organizations to Accelerate Science, Technology, and Medicine](https://www.dayoneproject.org/post/focused-research-organizations-to-accelerate-science-technology-and-medicine)

[^fn274]: \<sidenote\> See <https://www.darpa.mil/work-with-us/heilmeier-catechism>

[^fn275]: Speculatively, it might be possible to improve on the Heilmeier Catechism with something more quantized - similar to Donald Braben's BRAVERI scale.

[^fn276]: \<sidenote\> See[ DARPA - Enabling Technical Innovation by Jitendra Ranka](https://www.openbookpublishers.com/htmlreader/978-1-78374-791-7/ch10.xhtml)

[^fn277]: You also need some amount of slack for these benefits to kick in. If everybody is spending 110% of the their time focused on only their own programs any non-program-related interaction is mentally expensive.

[^fn278]: \<sidenote\> See T[he density of remaining nerve endings in human skin with and without postherpetic neuralgia after shingles](https://doi.org/10.1016/S0304-3959(00)00481-4)

[^fn279]: \<sidenote\> See[ The BioTac - Multimodal Tactile Sensor](https://wiki.ros.org/BioTac)

[^fn280]: \<sidenote\> See the[ Drexler-Smalley debate](https://en.wikipedia.org/wiki/Drexler%E2%80%93Smalley_debate_on_molecular_nanotechnology) for strong arguments against positional chemistry.

[^fn281]: Note that I'm *not* saying this is necessarily the right thing to build, but it does seem promising.

[^fn282]: \<sidenote\> See [DNA Strand Displacement-Driven DNA Origami Tools and Materials](https://www.energy.gov/sites/prod/files/2019/09/f66/DNA%20Strand%20Displacement%20Driven%20Molecular%20Additive%20Manufacturing.pdf).

[^fn283]: \<sidenote\> See [**Design of a single-chain polypeptide tetrahedron assembled from coiled-coil segments**](https://www.nature.com/articles/nchembio.1248)**.**

[^fn284]: \<sidenote\> See [Ultrafast rotation in an amphidynamic crystalline metal organic framework](https://www.pnas.org/content/pnas/114/52/13613.full.pdf).

[^fn285]: \<sidenote\> See [Casting inorganic structures with DNA molds.](https://science.sciencemag.org/content/346/6210/1258361.full)

[^fn286]: \<sidenote\> This term originally comes from another DARPA-riff, Activate. See [In the Realm of the Barely Feasible](https://issues.org/wp-content/uploads/2020/09/34-40-Prabhakar-In-the-Realm-of-the-Barely-Feasible-Fall-2020-ISSUES.pdf).

[^fn287]: This distinction between leading indicator and goal in of itself is of course the root of Goodhart's law. We can hopefully sidestep the trap of leading indicators becoming goals by having too many indicators to pursue all of them without just pursuing the thing they indicate!

[^fn288]: \<sidenote\> See [Reflections on 2020 as an independent researcher](https://andymatuschak.org/2020/)

[^fn289]: \<sidenote\> See [**Scenius, or Communal Genius**](https://kk.org/thetechnium/scenius-or-comm/)

[^fn290]: While Xerox PARC technically still exists, after a leadership change in 1983 it arguably morphed into a different organization.

[^fn291]: This is a yet-to-be-made tracker of how close we are to the technologies in Ian M. Banks Culture Series.

[^fn292]: \<sidenote\> A scale ranking civilizational levels by the amount of energy they can consume - Level 1 is consuming energy at the scale of all the energy incident to the surface of their planet, Level 2 is consuming energy at the scale of their local star, and Level 3 is consuming energy at the scale of their galaxy. See [Kardashev scale - Wikipedia](https://en.wikipedia.org/wiki/Kardashev_scale).

[^fn293]: \<sidenote\>'Ergophobia" technically means a neurotic fear of doing work, in an individual. But it comes from the same Greek root as "energy," and you will find that physicists use "work" and "energy" to mean the same thing. So I feel justified in using it to refer to the funk our society has gotten into over energy.' From: *Where Is My Flying Car?: A Memoir of Future Past.*

[^fn294]: \<sidenote\>"When a elder preeminent scientist says that something is impossible, he's probably wrong. When a elder preeminent scientist says something might be possible, he's probably right."

[^fn295]: the P is for private, but you can also imagine that it stands for Phenomenal or Prodigious if you want